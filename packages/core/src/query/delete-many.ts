...
import type { MutationProps as InvalidatesProps, TargetValues as InvalidateTargetValues, ResolvedMutationProps as ResolvedInvalidatesProps } from './invalidate'
...
import { Target as InvalidateTarget, resolveProps as resolveInvalidateProps, triggerRules as triggerInvalidates } from './invalidate'
...
> = Simplify<
	...
	& MutationModeProps
	& OptimisticUpdateProps<TData, TParams | undefined, DeleteManyProps<TParams>['ids']>
>
...
>(
	...
	function createExecuteFn(
		...
		const deleteMany = getSafeFetcherFn(resolvedProps, fetchers, 'deleteMany') as DeleteManyFn<TData, TParams>
		if (deleteMany != null)
			return () => deleteMany(resolvedProps, context)
		const deleteOne = getFetcherFn(resolvedProps, fetchers, 'deleteOne') as DeleteOneFn<TData, TParams>
		return () => fakeMany(resolvedProps.ids.map(id => deleteOne({ ...resolvedProps, id }, context)))
	}
}
	...
	if (shouldApplyOptimisticUpdate(listOptimisticUpdate)) {
		queryClient.setQueriesData(
			...
			typeof listOptimisticUpdate === 'function'
				? createOptimisticUpdaterFn(listOptimisticUpdate, props.params, props.ids)
				: createRemoveListItemUpdaterFn<TData, TPageParam, DeleteManyProps<any>['ids']>(props.ids),
		)
	}
	if (shouldApplyOptimisticUpdate(manyOptimisticUpdate)) {
		queryClient.setQueriesData(
			...
			typeof manyOptimisticUpdate === 'function'
				? createOptimisticUpdaterFn(manyOptimisticUpdate, props.params, props.ids)
				: createRemoveManyUpdaterFn<TData, DeleteManyProps<any>['ids']>(props.ids),
		)
	}
		...
		for (const id of props.ids) {
			if (typeof oneOptimisticUpdate === 'function') {
				queryClient.setQueriesData(
					...
					createOptimisticUpdaterFn(oneOptimisticUpdate, props.params, id),
				)
			}
			else {
