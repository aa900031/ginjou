...
Most composables differ only by the shape of the request object they send to the fetcher. Knowing these core shapes makes the rest of the API easier to learn.

| Operation | Required Fields | Optional Fields | Notes |
| :--- | :--- | :--- | :--- |
| `useGetOne` | `resource`, `id` | `meta` | Fetch one record. |
| `useGetList` | `resource` | `pagination`, `filters`, `sorters`, `meta` | Best for tables and index pages. |
| `useCreateOne` | `resource` | `meta` at hook level, `params` at mutate level | `params` is the payload to create. |
| `useUpdateOne` | `resource`, `id` | `params`, `meta` | `params` is the partial or complete update payload. |
| `useDeleteOne` | `resource`, `id` | `params`, `meta` | `params` is useful when the backend requires extra delete context. |

The `meta` object is intentionally generic. Use it to pass backend-specific options without forcing every provider into the same shape.
...
::tip
`meta` is also part of the query key. When `meta` changes, Ginjou treats the request as a different query and caches it separately.
::

### Implementing a Custom Fetcher

Pre-built providers are convenient, but the underlying contract is small enough to implement yourself.

```typescript
import type { Fetcher, GetListProps, GetOneProps } from '@ginjou/core'

export const customFetcher: Fetcher = {
	async getList(props) {
		const url = new URL(`/api/${props.resource}`, window.location.origin)

		if (props.pagination) {
			url.searchParams.set('page', String(props.pagination.current))
			url.searchParams.set('perPage', String(props.pagination.perPage))
		}
	...
	async getOne(props) {
		const response = await fetch(`/api/${props.resource}/${props.id}`)
		return {
...
Start with the methods your app actually needs. You do not have to implement every fetcher method on day one.
...
Ginjou also defines a richer operator set in `@ginjou/core`, including negated string operators, null checks, and logical operators.

| Category | Operators |
| :--- | :--- |
| Equality | `eq`, `ne`, `in`, `nin`, `null`, `nnull` |
| Numeric / range | `lt`, `lte`, `gt`, `gte`, `between`, `nbetween` |
| String | `contains`, `ncontains`, `containss`, `ncontainss`, `startswith`, `endswith` |
| Logical | `or`, `and` |

::tip
The operator exists at the framework level, but each provider decides how much of that operator set it can translate. For example, the REST provider intentionally supports a smaller subset than Supabase.
::

**Using `meta` With List Queries**

Use `meta` when the provider needs extra instructions that do not fit into generic `filters`, `sorters`, or `pagination`.
...
::tip
If your UI component expects a flat array, derive one explicitly with `records.flat()` or a computed property. Keep the original nested structure when you still need page boundaries for separators, virtual scrolling, or debug output.
::
...
When a query or mutation fails, Ginjou dispatches an error notification automatically. The notification includes the error message and is displayed via your [Notification provider](/guides/notifications).
...
Every data query composable (`useGetOne`, `useGetList`, `useGetMany`, `useGetInfiniteList`) integrates with `useCheckError` from the [Authentication](/guides/authentication) system. When a query fails:
