...
Use `optimisticUpdateMap` when you need to skip or customize those cache updates.
...
Mapper functions receive `(previous, params, target)`. For `list` and `many`, `target` is the `id` or `ids` passed to the mutation. For `one`, `target` is always a single id.
