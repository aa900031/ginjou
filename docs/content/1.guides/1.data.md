...
Object rules use the mutation's `fetcherName` when omitted, but not its `meta`; pass `meta` on the rule when the target queries use one. Use `useInvalidate` outside a mutation:

```ts
import { useInvalidate } from '@ginjou/vue' // or @ginjou/svelte
