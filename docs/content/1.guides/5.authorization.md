...
### Implementing `authz.access`

`useCanAccess` is only half of the workflow. The other half is implementing `authz.access` so the provider can evaluate the request context and return a result that the UI can trust.
...
The return value must include `can`. Add extra metadata only if your application needs it for audit logs, tooltips, or analytics.
...
const authz = {
	access: async ({ resource, action, params }) => {
		const user = await getCurrentUser()
		const data = await getData(resource, params.id)
