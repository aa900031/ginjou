...
Use an `Authz` provider to define rules for your application. This provider handles access control checks and permission retrieval.
...
<template>
  <button v-if="canAccess?.can">Edit Post</button>
</template>
```
