...
Use `params` for record-specific facts and `meta` for provider-specific policy inputs.
...
<script setup lang="ts">
import { defineAuthz } from '@ginjou/core'
import { defineAuthzContext, useCanAccess } from '@ginjou/vue'
import { computed } from 'vue'

const currentUser = { id: 7, roles: ['editor'] }

defineAuthzContext(defineAuthz({
	access({ action, resource, params, meta }) {
		if (resource === 'posts' && action === 'edit') {
			const ownsRecord = params?.ownerId === currentUser.id
			const editorOverride = meta?.allowEditors === true && currentUser.roles.includes('editor')

			return {
				can: ownsRecord || editorOverride,
				reason: ownsRecord || editorOverride ? undefined : 'Only the owner can edit this post.',
			}
		}
...
const canEdit = computed(() => editAccess.data.value?.can ?? false)
</script>
...
<script lang="ts">
import { defineAuthz } from '@ginjou/core'
import { defineAuthzContext, useCanAccess } from '@ginjou/svelte'
...
const canEdit = $derived(editAccess.data?.can ?? false)
</script>
