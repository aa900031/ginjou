...
The `meta` object is where resource-specific behavior lives.

| Field | Purpose |
| :--- | :--- |
| `parent` | Declares a parent resource for hierarchical navigation and breadcrumb building. |
| `hide` | Hides a resource from generated navigation while keeping it resolvable. |
| `deletable` | Marks whether the resource supports delete flows. |
| `fetcherName` | Selects a non-default fetcher for that resource. |
...
This pattern also scales to deeper trees. A three-level setup such as organizations → teams → members works the same way: each child resource points to its direct parent through `meta.parent`.
...
```vue [Vue]
<script setup lang="ts">
import { defineFetchersContext, defineResourceContext } from '@ginjou/vue'
import { graphqlFetcher, restFetcher } from './fetchers' // example imports
