...
Set `warnUnsaved: true` on `useCreate` or `useEdit` to prevent route navigation while the form has unsaved changes.

Ginjou does not decide when a form is dirty. Set `warnUnsavedActive` to `true` when the user changes editable state. A successful `save()` clears it automatically; a failed save leaves it active.

Only navigations that leave the page are confirmed. A change of query or hash keeps the same route mounted, so nothing is torn down and the form is left alone — paging a list or changing a filter never asks. Closing or reloading the tab does ask, through the browser's own confirmation. See [which navigations reach the predicate](/guides/router#which-navigations-reach-it) if your page needs a different rule.

The default confirmation uses the browser's native confirmation dialog. Pass `warnUnsaved: { confirm }` when your app needs a custom confirmation flow.
...
<template>
	<form @submit.prevent="submit">
		<input v-model="formData.title" @input="createPage.warnUnsavedActive.value = true">
		<button :disabled="createPage.isLoading">
			Create
		</button>
	</form>
...
async function submit(e: SubmitEvent) {
	e.preventDefault()
	await createPage.save({ title })
}
</script>

<form onsubmit={submit}>
	<input bind:value={title} oninput={() => createPage.warnUnsavedActive = true}>
	<button disabled={createPage.isLoading}>
		Create
	</button>
</form>
...
Set the guard only after copying the loaded record into local form state. Otherwise the initial record load would be treated as an edit.

`mutationMode` decides when the warning turns off.

| Mode | Warning turns off | If save fails |
| --- | --- | --- |
| `pessimistic` (default) | After save succeeds | Stays on |
| `optimistic` / `undoable` | When saving starts | Stays off |
...
<template>
	<form v-if="editPage.record" @submit.prevent="submit">
		<input v-model="formData.title" @input="editPage.warnUnsavedActive.value = true">
		<button :disabled="editPage.isLoading">
			Update
		</button>
	</form>
...
async function submit(e: SubmitEvent) {
	e.preventDefault()
	await editPage.save({ title })
}
</script>

{#if editPage.record}
	<form onsubmit={submit}>
		<input bind:value={title} oninput={() => editPage.warnUnsavedActive = true}>
		<button disabled={editPage.isLoading}>
			Update
		</button>
	</form>
...
Both accept a dot path or a getter function.

| Prop | Accepts | Default |
| --- | --- | --- |
| `labelKey` | `string` path or `(item) => label` | `'title'` |
| `valueKey` | `string` path or `(item) => value` | `'id'` |
| `selectedOptionsOrder` | `'in-place'` or `'selected-first'` | `'in-place'` |

`value` is the selected record key(s) — the same values `valueKey` puts on each option, which is what the selected records are fetched by. `valueKey` is therefore constrained to `string | number`; a getter returning anything else is a type error.

A getter `labelKey` has no field name to search on, so pass `searchKey` (or `searchToFilters`) when you use one — otherwise the search filter falls back to `'title'`.

`selectedOptionsOrder` decides where the selected value data lands when it is merged with the list page. `in-place` keeps the list order and appends missing selected items. `selected-first` puts the selected items at the top.
...
By default, `useSelect` builds a `contains` filter on `searchKey`. When `searchKey` is not set, it falls back to `labelKey` if that is a string, and to `title` otherwise. You can override the whole thing with `searchToFilters`.
