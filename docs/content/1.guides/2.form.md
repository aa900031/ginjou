...
| Mode | Request Timing | Navigation Timing | Failure Behavior | Typical Use |
| :--- | :--- | :--- | :--- | :--- |
| `pessimistic` | Wait for the server response first. | After success. | User stays on the page and sees the error. | Critical updates where accuracy matters more than speed. |
| `optimistic` | Fire immediately in the background. | Immediately. | UI may need to recover if the request fails later. | Fast CRUD flows where eventual correction is acceptable. |
| `undoable` | Delayed until the undo window closes. | Immediately. | User can cancel before the mutation is sent. | Destructive or high-risk actions where accidental clicks are common. |

::tip
`undoable` flows depend on a notification provider because the undo action is exposed through the notification UI. If you do not register `defineNotificationContext`, prefer `pessimistic` mode.
::
...
Keep fetched data and editable form data separate. `record` reflects server state, while `formData` reflects local user input. If you bind inputs directly to `record`, you blur the line between cached query data and unsaved edits.

::tip
Only copy the record into form state when you want to initialize or reset the form. If the record is refetched while the user is editing, decide explicitly whether to preserve local edits or replace them with the latest server response.
::
...
const { mutateAsync: deleteOne, isPending } = useDeleteOne()
		...
		<div v-if="showConfirmModal" class="modal-overlay">
			<div class="modal">
				...
				<div class="modal-actions">
					<button :disabled="isPending" class="btn-danger" @click="confirmDelete">
						{{ isPending ? 'Deleting...' : 'Delete' }}
					</button>
					<button :disabled="isPending" @click="cancelDelete">
						Cancel
					</button>
				</div>
			</div>
...
const options = [
	...
	{ label: 'Electronics', value: 5, data: { id: 5, name: 'Electronics' } },
...
The array is automatically deduplicated, so if "Electronics" appears in both results, it only shows once.
