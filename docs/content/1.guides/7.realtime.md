...
There are three realtime modes: **Off**, **Auto**, and **Manual**.

| Mode | How It Works | When to Use |
|------|-------------|-----------|
| **Off** | No subscription is created for the query | Pages where realtime traffic is unnecessary or too expensive |
...
In auto mode, Ginjou invalidates the current resource scope when an event arrives. In manual mode, Ginjou skips that automatic invalidation and gives you the event payload directly.
...
The callback receives a `RealtimeEvent` with this shape:

| Field | Description |
| :--- | :--- |
| `channel` | The channel that emitted the event, such as `resources/posts`. |
| `action` | Usually `created`, `updated`, `deleted`, or a custom string. |
| `payload` | Provider-specific event data. Many built-in flows include changed record IDs here. |
| `date` | Event timestamp. |
| `meta` | Optional backend-specific metadata, such as `fetcherName`. |
...
::tip
Choose manual mode when different event types should trigger different side effects. For example, you might refetch the list after `created`, patch local UI state after `updated`, and redirect the user after `deleted`.
::
