...
`records` is intentionally a nested array. Each outer item represents one fetched page. This keeps page boundaries available for debug output, per-page separators, and incremental rendering.

If your design system wants a flat array, derive it explicitly:
...
| Mode | Where Work Happens | Best For | Tradeoff |
| :--- | :--- | :--- | :--- |
| `server` | Backend | Large datasets and production APIs | Requires backend pagination support. |
| `client` | Browser | Small datasets already loaded in memory | The client must hold the full result set. |
| `off` | No pagination | Small reference lists or exports | Rendering cost can grow quickly. |

::tip
Prefer `server` unless you know the dataset is small and stable. `client` mode is convenient for short lists such as status options, lightweight catalogs, or demo data. `off` is best reserved for intentionally unpaginated views.
::
