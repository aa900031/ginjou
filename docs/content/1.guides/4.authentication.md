...
The exact `login()` payload depends on the auth provider. For example, the Supabase provider supports `password`, `oauth`, `otp`, `otp-token`, `idtoken`, and `sso`, while the Directus provider supports `password` and `sso`.
...
::tip
`useGetIdentity` is available only when your auth provider implements `getIdentity`. If your backend does not expose a dedicated identity endpoint yet, you can omit it and still use login, logout, and auth checks.
::
