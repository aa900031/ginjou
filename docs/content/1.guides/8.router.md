...
::note
With Vue Router, `isLeavingPath` is not always the right comparison. `/posts/1/edit -> /posts/2/edit` changes the path, but the route record is the same, so the component is reused while everything derived from the id refetches and rehydrates the form — you want to block that. A path can also change without the route doing so, between a record and its alias. `isChangingRoute` from `@ginjou/with-vue-router` answers with vue-router's own notion of the same route, reading the normalized locations out of `meta`:

```ts
import { isChangingRoute } from '@ginjou/with-vue-router'

const blocker = useRouteBlocker({
	shouldBlock: ({ currentLocation, nextLocation }) => isDirty.value && (
		nextLocation == null
		|| isChangingRoute(nextLocation.meta.location, currentLocation.meta.location)
	),
})
```
::
...
Vue Router needs nothing: `createRouter()` subscribes to the navigation outcomes itself.

svelte-spa-router reports them as props on the `<Router>` component, which only your app renders, so the adapter hands them out for you to wire up alongside `withBlocker`:
...
<Router
	routes={router.withBlocker(routes)}
	onRouteLoaded={router.onRouteLoaded}
	onConditionsFailed={router.onConditionsFailed}
/>
```

`withBlocker` is what lets a navigation be held at all. `onRouteLoaded` and `onConditionsFailed` are what move a blocker off `proceeding` at the moment the navigation really ends — either the new route loaded, or one of your own route conditions rejected it.

Leaving the two handlers out does not break blocking, and cancelling still settles on its own. What lingers is a blocker that approved: it stays `proceeding` until the next navigation attempt clears it. For the usual blocker — on the page being left — that is invisible, because the page is unmounted by then. It shows for a blocker that outlives the navigation, on a layout or an app shell.

If you pass your own handlers too, call the router's as well:
...
<template>
	<button type="button" @click="isRecording = !isRecording">
		{{ isRecording ? 'Stop recording' : 'Start recording' }}
	</button>
	<div v-if="blocker.state.value === 'blocked'">
		<p>Leave while recording?</p>
		<button type="button" @click="blocker.proceed?.()">
			Leave anyway
		</button>
		<button type="button" @click="blocker.reset?.()">
			Keep recording
		</button>
	</div>
</template>
