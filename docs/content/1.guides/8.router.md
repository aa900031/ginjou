...
| `blocker` | Optionally register a callback that can hold a navigation. |
...
```ts [RouterBlocker]
type RouterBlockerFn = (shouldBlock: RouterBlockShouldFn) => RouterBlockerHandle
...
| State | Meaning |
| --- | --- |
| `unblocked` | No navigation is waiting. |
| `blocked` | Navigation is waiting for a decision. |
| `proceeding` | Navigation is allowed and in progress. |

### Actions

| Method | What it does | State after calling it |
| --- | --- | --- |
| `proceed()` | Continue the waiting navigation. Available only while `blocked`. | `proceeding`, then `unblocked` after the location changes. |
| `reset()` | Cancel the waiting navigation. Available only while `blocked`. | `unblocked` |
