...
| `createMany` | Supported | One `createOne` request per record, run in parallel. |
| `updateMany` | Supported | One `updateOne` request per id, run in parallel. |
| `deleteMany` | Supported | One `deleteOne` request per id, run in parallel. |

json-server has no bulk endpoints, so the `*Many` mutations fan out one request per record and return the responses in the same order.
...
| `createMany` | `POST` | `{url}/{resource}` per record | none | each item of `params` |
| `updateMany` | `PUT` | `{url}/{resource}/{id}` per id | none | `params` |
| `deleteMany` | `DELETE` | `{url}/{resource}/{id}` per id | none | optional `params` |
...
| Meta field | Used by |
| --- | --- |
| `meta.method` | `getList`, `getOne`, `getMany`, `createOne`, `createMany`, `updateOne`, `updateMany`, `deleteOne`, `deleteMany` |
| `meta.headers` | `getList`, `getOne`, `getMany`, `createOne`, `createMany`, `updateOne`, `updateMany`, `deleteOne`, `deleteMany` |
