...
The channel `resources/<name>` maps to the table `<name>`.
...
| `payload.data` | `new` for insert and update, `old` for delete |
...
| --- | --- | --- |
| `schema` | `'public'` | Postgres schema of the table |
| `idColumnName` | `'id'` | Column used for `one` and `many` filters |

`publish` is not implemented. Postgres already broadcasts every change, so mutations do not need to publish their own events.
