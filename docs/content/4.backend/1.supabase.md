---
title: Supabase
description: Explain the fetcher mapping, auth, and realtime integration in @ginjou/with-supabase.
---

`@ginjou/with-supabase` provides a Ginjou fetcher, auth provider, and realtime provider for Supabase.

Most apps pass the same Supabase client to all of them.

It does not change the higher-level Ginjou API. It only connects those hooks to the Supabase JavaScript client.

## Installation

Install `@supabase/supabase-js` together with the adapter.

::code-group
---
sync: package-manager
---

```bash [pnpm]
pnpm add @ginjou/with-supabase @supabase/supabase-js
```

```bash [yarn]
yarn add @ginjou/with-supabase @supabase/supabase-js
```

```bash [npm]
npm install @ginjou/with-supabase @supabase/supabase-js
```

```bash [bun]
bun add @ginjou/with-supabase @supabase/supabase-js
```

::

| Package | Supported version |
| --- | --- |
| `@supabase/supabase-js` | `^2.7.0` |

## Fetcher

Use `createFetcher()` with a Supabase client.

| Prop | Required | Meaning |
| --- | --- | --- |
| `client` | Yes | A Supabase client instance. |

Register it through `defineFetchersContext()`.

::code-group

```ts [Vue]
import { defineFetchersContext } from '@ginjou/vue'
import { createFetcher } from '@ginjou/with-supabase'
import { createClient } from '@supabase/supabase-js'

const supabase = createClient(
	'https://your-project.supabase.co',
	'your-anon-key',
)

defineFetchersContext({
	default: createFetcher({ client: supabase }),
})
```

::

The adapter implements every fetcher method: `getList`, `getMany`, `getOne`, `createOne`, `createMany`, `updateOne`, `updateMany`, `deleteOne`, `deleteMany`, and `custom`.

`updateMany` and `deleteMany` match rows with `in(idColumn, ids)` and return the affected rows through `select(meta.select ?? '*')`.

### Pagination

List pagination uses Supabase `range()`.

| Ginjou input | Supabase behavior |
| --- | --- |
| `pagination.current` | Used to calculate the starting row |
| `pagination.perPage` | Used to calculate the ending row |

The adapter calls:

| Range argument | Value |
| --- | --- |
| `from` | `(current - 1) * perPage` |
| `to` | `current * perPage - 1` |

For totals, `getList()` uses Supabase count mode from `meta.count` and returns `count || 0`.

### Filters

The adapter maps Ginjou filters to Supabase PostgREST filter helpers.

| Ginjou operator | Supabase behavior |
| --- | --- |
| `eq` | `eq(field, value)` |
| `ne` | `neq(field, value)` |
| `in` | `in(field, value)` |
| `gt` | `gt(field, value)` |
| `gte` | `gte(field, value)` |
| `lt` | `lt(field, value)` |
| `lte` | `lte(field, value)` |
| `contains` | `ilike(field, %value%)` |
| `containss` | `like(field, %value%)` |
| `null` | `is(field, null)` |
| `startswith` | `ilike(field, value%)` |
| `endswith` | `ilike(field, %value)` |
| `or` | `or(...)` with joined logical clauses |

These operators use the generic `filter()` fallback.

| Ginjou operator | Generic fallback operator |
| --- | --- |
| `nin` | `not.in` |
| `ncontains` | `not.ilike` |
| `ncontainss` | `not.like` |
| `nnull` | `not.is` |

These operators are not supported and throw instead of being translated.

| Operator | Behavior |
| --- | --- |
| `and` | Throws an error |
| `between` | Throws an error |
| `nbetween` | Throws an error |

### Sorters

Sorters are applied through Supabase `order()`.

Normal fields use `order(field, { ascending })`.

Related table fields can use dot notation.

| Ginjou sorter | Supabase behavior |
| --- | --- |
| `{ field: 'title', order: 'asc' }` | `order('title', { ascending: true })` |
| `{ field: 'profile.name', order: 'desc' }` | `order('name', { ascending: false, foreignTable: 'profile' })` |

When dot notation is used, the adapter also expands the select string so Supabase can order through the related table.

### Meta

These meta fields are verified in the adapter.

| Meta field | Used by | Default | Purpose |
| --- | --- | --- | --- |
| `select` | All fetcher methods | `'*'` where applicable | PostgREST select expression |
| `count` | `getList` | `'exact'` | Count strategy for pagination total |
| `idColumnName` | `getMany`, `getOne`, `updateOne`, `updateMany`, `deleteOne`, `deleteMany` | `'id'` | Custom primary key column |

Use `meta.idColumnName` when the record key is not stored in an `id` column.

::code-group

```ts [meta.id-column-name.ts]
import { useGetOne } from '@ginjou/vue'

useGetOne({
	resource: 'orders',
	id: 'order-001',
	meta: {
		idColumnName: 'order_id',
	},
})
```

::

Use `meta.select` to control the returned columns and relations.

::code-group

```ts [meta.select.ts]
import { useGetList } from '@ginjou/vue'

useGetList({
	resource: 'posts',
	meta: {
		select: 'id,title,author:users(id,name)',
	},
})
```

::

Use `meta.count` to choose the Supabase count mode for `getList()`.

::code-group

```ts [meta.count.ts]
import { useGetList } from '@ginjou/vue'

useGetList({
	resource: 'posts',
	meta: {
		count: 'planned',
	},
})
```

::

### Custom

`custom()` routes on the `url` prefix.

| `url` | Supabase call |
| --- | --- |
| `rpc/<name>` | `client.rpc(name, payload, { get, head })`, then `filters` and `sorters` are applied to the result |
| anything else | `client.functions.invoke(url, { method, body: payload, headers })`, with `query` appended as a search string |

Call a Postgres function:

::code-group

```ts [rpc.ts]
import { useCustom } from '@ginjou/vue'

useCustom({
	url: 'rpc/search_posts',
	method: 'post',
	payload: { q: 'hello' },
	sorters: [{ field: 'title', order: 'asc' }],
})
```

::

Invoke an Edge Function:

::code-group

```ts [edge-function.ts]
import { useCustomMutation } from '@ginjou/vue'

const { mutate } = useCustomMutation()

mutate({
	url: 'send-email',
	method: 'post',
	payload: { to: 'user@example.com' },
})
```

::

## Auth

Use `createAuth()` with the same Supabase client.

Most apps reuse the client they already passed to `createFetcher()`.

Register it through `defineAuthContext()`.

::code-group

```ts [Vue]
import { defineAuthContext } from '@ginjou/vue'
import { createAuth } from '@ginjou/with-supabase'
import { createClient } from '@supabase/supabase-js'

const supabase = createClient(
	'https://your-project.supabase.co',
	'your-anon-key',
)

defineAuthContext(createAuth({ client: supabase }))
```

::

### Login

`login()` dispatches to one of the Supabase auth client methods based on `type`.

| Login type | Supabase client call |
| --- | --- |
| `password` | `signInWithPassword()` |
| `oauth` | `signInWithOAuth()` |
| `idtoken` | `signInWithIdToken()` |
| `otp` | `signInWithOtp()` |
| `sso` | `signInWithSSO()` |
| `otp-token` | `verifyOtp()` |

Password login:

::code-group

```ts [Vue]
import { useLogin } from '@ginjou/vue'

const { mutateAsync: login } = useLogin()

await login({
	type: 'password',
	params: {
		email: 'user@example.com',
		password: 'password123',
	},
})
```

::

OAuth login:

::code-group

```ts [Vue]
import { useLogin } from '@ginjou/vue'

const { mutateAsync: login } = useLogin()

await login({
	type: 'oauth',
	params: {
		provider: 'github',
	},
})
```

::

### Logout

`logout()` maps to `client.auth.signOut()`.

It forwards the optional logout params to Supabase.

### Identity

`getIdentity()` maps to `client.auth.getUser()`.

It returns the Supabase user object.

### Check Authentication

`check()` maps to `client.auth.getSession()`.

| Session state | Result |
| --- | --- |
| Valid session | `{ authenticated: true }` |
| No valid session and no auth error | `{ authenticated: false, logout: false, error: null }` |
| Auth error | `{ authenticated: false, logout: true, error }` |

Auth failures are returned as part of the result instead of being thrown.

### Check Error

`checkError()` uses Supabase `isAuthError()`.

| Error kind | Result |
| --- | --- |
| Auth error | `{ logout: true, error }` |
| Other error | `{}` |

## Realtime

Use `createRealtime()` with the same Supabase client. It listens to [Postgres Changes](https://supabase.com/docs/guides/realtime/postgres-changes), so the table must be added to the `supabase_realtime` publication.

Register it through `defineRealtimeContext()`.

::code-group

```ts [Vue]
import { defineRealtimeContext } from '@ginjou/vue'
import { createRealtime } from '@ginjou/with-supabase'
import { createClient } from '@supabase/supabase-js'

const supabase = createClient(
	'https://your-project.supabase.co',
	'your-anon-key',
)

defineRealtimeContext(createRealtime({ client: supabase }))
```

::

### Channels and Actions

The table is `params.resource`. Without params, the channel `resources/<name>` maps to the table `<name>`.

Unknown actions are ignored.

| Ginjou action | Postgres event |
| --- | --- |
| `created` | `INSERT` |
| `updated` | `UPDATE` |
| `deleted` | `DELETE` |
| `*` | `*` |

### Filters

Subscriptions narrow the server-side stream when the query params allow it.

| Subscription params | Realtime filter |
| --- | --- |
| `{ type: 'one', id }` | `<idColumn>=eq.<id>` |
| `{ type: 'many', ids }` | `<idColumn>=in.(<ids>)` |
| `{ type: 'list', filters }` | First logical filter on a plain column using `eq`, `ne`, `lt`, `lte`, `gt`, `gte`, or `in` |

Supabase Realtime accepts a single filter with those operators, on a column of the subscribed table, and the value must be a string, number, or boolean. A filter on a dotted path into an embedded resource is skipped, because the server would reject the whole subscription over it. An `in` list, including the ids of a `many` subscription, additionally rejects values containing a comma, because the server would read them as several separate values. Anything else is ignored and the subscription receives the whole table.

Supabase does not filter `DELETE` events on the server, so `one` and `many` subscriptions also check the record id on the client. A `many` subscription with no ids opens no channel at all.

A `list` filter also narrows `UPDATE`, so a record that leaves the filtered list is not reported. In `manual` mode, re-check membership in your callback.

### Event Payload

| Field | Value |
| --- | --- |
| `payload.ids` | `[record[idColumn]]`, or `undefined` when the record has no id |
| `payload.data` | `new` for insert and update, `old` for delete (primary key only unless the table has `REPLICA IDENTITY FULL`) |
| `date` | Postgres commit timestamp |

### Meta

| Meta field | Default | Purpose |
| --- | --- | --- |
| `schema` | The client's `db.schema`, then `'public'` | Postgres schema of the table |
| `idColumnName` | `'id'` | Same field as the fetcher meta. Used for `one` and `many` filters, the client-side id check, and `payload.ids` |

### Errors

Realtime never throws. A subscription that Supabase rejects (table not in the publication, invalid filter) is reported with `console.warn`.

### Publish

`publish` is not implemented, so `usePublish` and `useCustomMutation({ publish })` are no-ops with this provider. Subscribers are notified only by Postgres Changes on tables in the `supabase_realtime` publication; changes made elsewhere (RPCs on unpublished tables, edge functions, external APIs) do not reach them.
