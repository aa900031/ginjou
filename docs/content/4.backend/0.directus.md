...
The adapter runs no scrubbing pass over your query, so `_eq: null`, `_eq: ''` and `_in: []` reach Directus unchanged. They are real conditions, and `_eq: null` in particular is how you ask for rows that were never soft-deleted. Dropping it would silently widen the query to the whole collection. Keys you leave as `undefined` are omitted by the Directus SDK itself.
