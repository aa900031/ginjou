...
`getMany` reads by `id` with an `_in` filter and sets `limit` to the number of ids. The filter goes into `_and`, so an `id` filter you pass through `meta.query.filter` is kept as well. `deleteMany` returns an empty array because Directus sends no body back.
...
| Empty `_and` / `_or` group | Dropped, from `filters` and from `meta.query.filter` alike. |
...
A logical group with no members is the one exception, and dropping it changes nothing. Directus turns `_or: []` into an empty SQL group that its query builder omits, so the query it runs is identical whether the group is sent or not. `_in: []` is the opposite case and is always sent: it compiles to `1 = 0` and matches no rows, so dropping it would silently turn "match nothing" into "match everything".
