...
`custom()` accepts `filters` and `sorters` and converts them with the same rules as above, sending them as the `filter` and `sort` query parameters. They are only attached when you pass them, since a custom `url` may point at an extension endpoint that does not read Directus's query convention.

Your own `query` is applied last, so a `filter` or `sort` you write there wins over the generated one.
