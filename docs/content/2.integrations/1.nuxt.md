...
There is no `useAsyncCreate`.

Create, update, and delete flows still use the normal mutation hooks such as `useCreateOne`, `useUpdateOne`, and `useDeleteOne`.

Mutation hooks do not have async versions because they are not SSR data-loading primitives.
