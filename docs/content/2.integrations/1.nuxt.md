...
Use `app/app.vue` or `app.vue` for provider registration. That keeps setup SSR-safe and consistent across server and client rendering.
...
::tip
Create the `QueryClient` once in the root component and keep provider registration unconditional. Conditional setup based on route or environment is a common cause of hydration mismatches.
::
...
Async composables preserve the same result shape as their Vue counterparts. For example, `useAsyncGetList` still returns `records`, `data`, `isFetching`, and the rest of the standard query state.
