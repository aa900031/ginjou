...
walkthrough, and [core/setup.md](../core/setup.md) for the provider model.
...
behavior live in the framework-agnostic `core/*` references — read those for
...
This skill has a Svelte reference per feature, mirroring `vue/` file-for-file
(`svelte/controllers.md`, `svelte/forms.md`, `svelte/data-composables.md`,
`svelte/authentication.md`, `svelte/authorization.md`, `svelte/notifications.md`,
`svelte/realtime.md`, `svelte/i18n.md`, `svelte/resources.md`, `svelte/router.md`).
Open the one matching your feature for its Svelte example; it delegates shared
behavior to the `vue/` and `core/` references.
...
- Pass an accessor function (`() => ({ … })`) for props that depend on reactive
  state.
- For composable options and behavior, read the matching `core/*` reference —
